...
class FeaturesEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        customer_embedding_final[:,9,:] = self.product_type_name_embedding(x[:,9])
        customer_embedding_final[:,10,:] = self.graphical_embedding(x[:,10])
        customer_embedding_final[:,11,:] = self.colour_embedding(x[:,11])
        customer_embedding_final[:,12,:] = self.department_embedding(x[:,12])
        customer_embedding_final[:,13,:] = self.index_group_name_embedding(x[:,13])
        customer_embedding_final[:,14,:] = self.FN_embedding(x[:,14])
        customer_embedding_final[:,15,:] = self.Active_embedding(x[:,15])
        customer_embedding_final[:,16,:] = self.club_member_status_embedding(x[:,16])
